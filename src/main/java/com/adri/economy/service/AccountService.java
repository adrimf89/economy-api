...
public class AccountService {
    ...
    public Optional<AccountDTO> findById(long id){
        return accountRepository.findById(id)
                .map(account -> Optional.of(Mapper.mapToAccountDTO(account)))
                .orElse(Optional.empty());
    }

    public Page<AccountDTO> findAll(Pageable pageable){
        return accountRepository.findAll(pageable)
                .map(account -> Mapper.mapToAccountDTO(account));
    }
    ...
    public AccountDTO createAccount(FormAccountDTO form) {
        ...
        return Mapper.mapToAccountDTO(accountRepository.save(account));
    }
}
